### Проблемы
- Не типизируется enteredComponentValue при указании типа предыдущему аргументу (schema: SomeComponentSchema). value тянется от extends дженерика.


### Фитчи
- Сейчас если в качестве операнда указать tempalte component, то в функции обработчике по нему можно получить только ВСЕ экземпляры данного template компонента. То есть если мы укажем template comp вложенного repeater, мы можем получить его экземпляры в рамках ВСЕЙ формы, а не в рамках какого-то родителя repeater выше по дереву.
...
- Оператор для типа repeater, без enterComponentValue и с типом RepeaterComponentProperties
