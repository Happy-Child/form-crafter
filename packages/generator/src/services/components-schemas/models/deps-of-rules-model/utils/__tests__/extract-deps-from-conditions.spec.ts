...
describe('extractDepsFromConditions', () => {
    it('should extract components deps from conditions 1', () => {
        const result = extractDepsFromConditions([], mockComponentSchema.email.mutations!.schemas[0].condition!)

        expect(result).toEqual(['input-first-name', 'date-birth'])
    })

    it('should extract components deps from conditions 2', () => {
        const result = extractDepsFromConditions([], mockComponentSchema['input-last-name'].mutations!.schemas[1].condition!)
